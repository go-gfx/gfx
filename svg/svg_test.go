...
// pixAt returns the RGBA at (x,y) of a result surface.
func pixAt(p *Result, x, y int) color.RGBA {
	i := (y*p.Image.W + x) * 4
	return color.RGBA{p.Image.Pix[i], p.Image.Pix[i+1], p.Image.Pix[i+2], p.Image.Pix[i+3]}
}
...
func TestRasterizeFixture(t *testing.T) {
	...
	wantH := int(math.Round(153.4 * 2))
	if p.Image.W != 978 || p.Image.H != wantH {
		t.Fatalf("dims = %dx%d, want 978x%d", p.Image.W, p.Image.H, wantH)
	}
	if len(p.Image.Pix) != p.Image.W*p.Image.H*4 {
		t.Fatalf("Pix len = %d, want %d", len(p.Image.Pix), p.Image.W*p.Image.H*4)
	}

	// Page must not be blank: some pixels differ from Paper.
	ink := 0
	for i := 0; i+3 < len(p.Image.Pix); i += 4 {
		if p.Image.Pix[i] != white.R || p.Image.Pix[i+1] != white.G || p.Image.Pix[i+2] != white.B {
			ink++
		}
	}
	if ink == 0 {
	...
	// Groups must be non-empty and every non-empty bound well formed and in range.
	if len(p.Groups) == 0 {
	...
	for _, g := range p.Groups {
		if g.Attrs == nil {
			t.Errorf("group has nil Attrs: %+v", g)
		}
		if g.Bounds.Empty() {
			continue
		}
		drew++
		if !g.Bounds.In(surf) {
			t.Errorf("group bounds %v not within surface %v", g.Bounds, surf)
		}
	}
	if drew == 0 {
		t.Fatal("no group recorded a non-empty bound")
	}
}

func regionHasInk(p *Result, x0, y0, x1, y1 int) bool {
	for y := y0; y < y1 && y < p.Image.H; y++ {
		for x := x0; x < x1 && x < p.Image.W; x++ {
			c := pixAt(p, x, y)
...
func TestPaperTransparentAndOpaque(t *testing.T) {
	...
	// An empty <g> drew nothing: it is still recorded, with an empty Bounds.
	if len(p.Groups) != 1 {
		t.Fatalf("want 1 group, got %d", len(p.Groups))
	}
	if !p.Groups[0].Bounds.Empty() {
		t.Fatalf("empty group has non-empty bounds: %v", p.Groups[0].Bounds)
	}
...
func TestGroupBoundsExact(t *testing.T) {
	// A <g id="box"> containing one axis-aligned rect. At scale 2 the rect user
	// box (4,6)-(12,11) maps to device (8,12)-(24,22); the coverage box is exact.
	doc := `<svg viewBox="0 0 20 20"><g id="box" class="marker"><rect x="4" y="6" width="8" height="5" fill="black"/></g></svg>`
	...
	g := p.Groups[0]
	if g.Attrs["id"] != "box" || g.Attrs["class"] != "marker" {
		t.Fatalf("attrs not captured: %v", g.Attrs)
	}
	want := image.Rect(8, 12, 24, 22)
	if g.Bounds != want {
		t.Fatalf("group bounds = %v, want %v", g.Bounds, want)
	}
}

func TestNestedGroupsBothBounded(t *testing.T) {
	// An outer <g> wraps two inner <g>s; the outer bound is the union of both.
	doc := `<svg viewBox="0 0 40 20"><g id="outer">` +
		`<g id="a"><rect x="2" y="2" width="4" height="4" fill="black"/></g>` +
		`<g id="b"><rect x="30" y="10" width="6" height="6" fill="black"/></g>` +
		`</g></svg>`
	...
	if len(by) != 3 {
		t.Fatalf("want 3 groups, got %d: %v", len(by), p.Groups)
	}
	if got, want := by["a"].Bounds, image.Rect(2, 2, 6, 6); got != want {
		t.Errorf("a bounds = %v, want %v", got, want)
	}
	if got, want := by["b"].Bounds, image.Rect(30, 10, 36, 16); got != want {
		t.Errorf("b bounds = %v, want %v", got, want)
	}
	// outer is the union of a and b.
	if got, want := by["outer"].Bounds, image.Rect(2, 2, 36, 16); got != want {
		t.Errorf("outer bounds = %v, want %v", got, want)
	}
}
...
func TestInkRecolouring(t *testing.T) {
	...
	for y := 0; y < pb.Image.H && !found; y++ {
		for x := 0; x < pb.Image.W; x++ {
			c := pixAt(pb, x, y)
...
func TestScaleDefault(t *testing.T) {
	...
	if p.Image.W != 20 || p.Image.H != 20 {
		t.Fatalf("default scale dims = %dx%d, want 20x20", p.Image.W, p.Image.H)
	}
}
...
func TestRootViewportFallback(t *testing.T) {
	...
	if p.Image.W != 20 || p.Image.H != 10 {
		t.Fatalf("fallback dims = %dx%d, want 20x10", p.Image.W, p.Image.H)
	}
	...
	if p2.Image.W != 8 || p2.Image.H != 4 {
		t.Fatalf("bad-viewBox fallback dims = %dx%d, want 8x4", p2.Image.W, p2.Image.H)
	}
}
...
func TestPathCommands(t *testing.T) {
	...
	for i, d := range docs {
		...
		if !regionHasInk(p, 0, 0, p.Image.W, p.Image.H) {
...
func TestTransformsAndNestedSVG(t *testing.T) {
	...
	if !regionHasInk(p, 0, 60, 25, p.Image.H) {
...
func TestEmbeddedImage(t *testing.T) {
	...
	doc := `<svg viewBox="0 0 20 20"><g id="pic"><image x="4" y="4" width="12" height="12" href="` + uri + `"/></g></svg>`
	...
	foundRed := false
	for y := 0; y < p.Image.H && !foundRed; y++ {
	...
	// The enclosing group's bounds must cover the blitted destination rectangle.
	if len(p.Groups) != 1 || p.Groups[0].Bounds.Empty() {
		t.Fatalf("image group did not record a bound: %v", p.Groups)
	}
}
...
func TestFillPathClampedOut(t *testing.T) {
	...
	// The group is recorded but drew nothing on-surface -> empty bounds.
	if len(p.Groups) != 1 || !p.Groups[0].Bounds.Empty() {
		t.Fatalf("off-surface group should have empty bounds: %v", p.Groups)
	}
}
...
func TestAddBoundsUnion(t *testing.T) {
	// Directly exercise addBounds: a shape inside two nested groups unions into
	// both; a rect that lands in no group is a no-op.
	r := &renderer{img: raster.New(50, 50)}
	r.groups = []Group{{Attrs: map[string]string{}}, {Attrs: map[string]string{}}}
	st := state{groups: []int{0, 1}}
	r.addBounds(st, image.Rect(2, 3, 6, 8))
	r.addBounds(st, image.Rect(4, 1, 10, 5))
	want := image.Rect(2, 1, 10, 8)
	if r.groups[0].Bounds != want || r.groups[1].Bounds != want {
		t.Fatalf("union = %v / %v, want %v", r.groups[0].Bounds, r.groups[1].Bounds, want)
	}
	// No enclosing group: nothing recorded, no panic.
	r.addBounds(state{}, image.Rect(0, 0, 5, 5))
}
